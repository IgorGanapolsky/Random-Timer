...
def run(
    ...
    payload: Dict[str, Any] = {
        ...
        "definitions": {
            "install_proxy_posthog": "Distinct persons with event Application Installed for pragmatic_live only; this is not store download truth.",
            "reviews_store": "Store API review signals. For final install/review truth, use App Store Connect / Google Play Console.",
            "reviews_in_app": "PostHog review_prompt_requested / write_review_tapped for pragmatic_live only; not published store star count.",
            "paid_posthog": "paywall_purchase_success and paywall_purchase_result are funnel signals only; use RevenueCat or Apple/Google server notifications for ledger truth.",
            "crashes": "Crashlytics via BigQuery export; PostHog $exception also listed if captured.",
            "uninstalls": "Not available on iOS in a per-user sense; use Play Console/reporting views for Android uninstall truth.",
        },
        "posthog": posthog,
