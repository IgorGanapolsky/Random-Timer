...
def evaluate(root: Path) -> dict[str, Any]:
    ...
    child["agentzip_memory"] = _run_json(root, "scripts/agentzip_memory_gate.py")
    if not child["agentzip_memory"].get("ready"):
        blockers.append("agentzip_memory_not_ready")

    child["apple_pcc"] = _run_json(root, "scripts/apple_pcc_gate.py")
    if not child["apple_pcc"].get("ready"):
        blockers.append("apple_pcc_not_ready")

    child["hydrafusion_routing"] = _run_json(root, "scripts/hydrafusion_routing_gate.py")
    ...
    child["semantic_search_stack"] = _run_json(
        root, "scripts/semantic_search_stack_gate.py"
    )
