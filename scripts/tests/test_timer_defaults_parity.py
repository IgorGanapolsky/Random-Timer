...
def test_timer_defaults_match_across_mobile_platforms():
    ...
    assert "private fun Preferences.toRawTimerConfig()" in android_repository
    assert "maxSeconds = this[KEY_MAX_SECONDS] ?: 300" in android_repository
    assert android_repository.count("preferences.toRawTimerConfig().clampedForPro()") == 2
