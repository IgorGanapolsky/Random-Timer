...
EOF
    ...
    while [ $session -lt $max_sessions ]; do
        ...
        # Update state
        jq ".sessions = $session | .tasks_completed = $completed" "$STATE_FILE" > "${STATE_FILE}.tmp" && mv "${STATE_FILE}.tmp" "$STATE_FILE"
        ...
        if echo "$output" | grep -q "RALPH_ALL_COMPLETE"; then
            ...
            log "${YELLOW}⚠ Task blocked: $reason${NC}"
...
${BLUE}How It Works:${NC}
  ...
  2. Loops through tasks, one per Claude session
  3. Each session: implement → test → commit → update plan
  4. Continues until all tasks complete
  5. Creates draft PR when done
