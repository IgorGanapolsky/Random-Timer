...
description: NVIDIA PAIR local inference router — proxy Ollama/LM Studio, schedule independent jobs across Mac/mini/S25 edge (docs/NVIDIA_PAIR_LOCAL_ROUTER.md). Upstream https://github.com/NVIDIA/Personal-AI-Router.
---
...
Rules:
1. Keep harness on PAIR proxy (`http://127.0.0.1:11434`) — expect process `ollama-pr`; no new cluster API.
2. One independent request → one eligible node (engine + exact model + ready).
3. Never claim multi-node without Jobs / `workloads-history.json` (`scheduledOn` >1).
4. No VRAM pooling / single-request sharding.
5. MacBook + Mac mini = native PAIR peers via Tailscale/LAN + Cluster PIN.
6. Galaxy S25 = Termux/Ollama **edge** only (`native_pair=false`).
7. No paid remote GPU under the $20/mo hard cap.
8. Do not run `PAIR.app` and `nvpair-tui` together.
