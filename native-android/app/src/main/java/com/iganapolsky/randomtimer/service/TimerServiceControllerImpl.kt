...
class TimerServiceControllerImpl
    ...
    constructor(
        ...
        override fun updateVoiceEnabled(enabled: Boolean) {
            val intent =
                Intent(context, TimerForegroundService::class.java).apply {
                    action = TimerForegroundService.ACTION_UPDATE_VOICE
                    putExtra(TimerForegroundService.EXTRA_VOICE_ENABLED, enabled)
                }
            context.startService(intent)
        }
